...
// IncrementalBackupDaemon implements BackupDaemonUseCase for incremental backups.
// start_ts is expected to be already resolved by BackupExecutor before EnqueueBackup is called.
type IncrementalBackupDaemon struct {
...
func NewIncrementalBackupDaemon(
	storageRepo repo.StorageRepository,
	dbRepo repo.DBRepository,
	scheduler SchedulerRepository,
	s3Client S3ClientRepository,
	...
	return &IncrementalBackupDaemon{
...
func (b *IncrementalBackupDaemon) ListBackups(_ context.Context, _ string) ([]string, error) {
	return b.storageRepo.ListVaultNames(false, repo.ALL, "")
}

func (b *IncrementalBackupDaemon) ListBackup(ctx context.Context, procType string, vaultPath string) (map[string]interface{}, error) {
	return b.GetBackupStats(ctx, vaultPath, "", "", procType)
}

func (b *IncrementalBackupDaemon) GetBackupStats(ctx context.Context, vaultName string, ts string, backupPath string, procType string) (map[string]interface{}, error) {
	...
	if vaultObj.IsGranular {
		...
		result["db_list"] = fmt.Sprintf("%s backup", INCREMENTAL)
	}
	...
	b.logger.Debugf("Incremental backup stats for %s: %+v", name, result)
	return result, nil
}

// EnqueueBackup enqueues an incremental backup.
// start_ts must already be set in request.CustomVars by BackupExecutor.
func (b *IncrementalBackupDaemon) EnqueueBackup(ctx context.Context, request entity.BackupRequest) (entity.BackupResponse, error) {
	if request.CustomVars == nil {
		request.CustomVars = make(map[string]string)
	}

	// Guard: start_ts must exist — set by BackupExecutor before delegation
	if request.CustomVars["start_ts"] == "" {
...
func (b *IncrementalBackupDaemon) RestoreBackup(ctx context.Context, request entity.RestoreRequest) (entity.RestoreResponse, error) {
	if request.CustomVars == nil {
		request.CustomVars = make(map[string]string)
	}
	...
	b.logger.Infof("Starting incremental restore from: %s, %s", request.ExternalBackupPath, vault.Folder)
...
func (b *IncrementalBackupDaemon) EnqueueEviction(_ context.Context, _ entity.EvictRequest) error {
	...
	if b.evictionPolicy != "" {
		...
		if err != nil {
			return fmt.Errorf("failed to list incremental vaults err: %w", err)
		}
		obsoleteFull, err := b.evict(fullVaults, b.evictionPolicy, excludedFiles)
		if err != nil {
			return fmt.Errorf("failed to evict incremental vaults err: %w", err)
		}
	...
	if b.granularEvictionPolicy != "" {
		...
		if err != nil {
			return fmt.Errorf("failed to list incremental granular vaults err: %w", err)
		}
		obsoleteGranular, err := b.evict(granularVaults, b.granularEvictionPolicy, excludedFiles)
		if err != nil {
			return fmt.Errorf("failed to evict incremental granular vaults err: %w", err)
		}
...
func (b *IncrementalBackupDaemon) RemoveBackup(ctx context.Context, request entity.EvictByVaultRequest) error {
...
func (b *IncrementalBackupDaemon) RemoveBackupV2(ctx context.Context, request entity.EvictByVaultV2Request) error {
...
func (b *IncrementalBackupDaemon) RemoveRestoreV2(ctx context.Context, request entity.EvictByVaultV2Request) error {
	backupID := strings.TrimSpace(request.Vault)
	if backupID == "" {
		return fmt.Errorf("vault is required")
	}
...
func (b *IncrementalBackupDaemon) GetJobStatus(ctx context.Context, request entity.JobStatusRequest) (entity.JobStatusResponse, error) {
	job, err := b.dbRepo.SelectEverything(ctx, request.TaskID)
	if err != nil {
...
func (b *IncrementalBackupDaemon) CreateS3PresignedURL(ctx context.Context, request entity.S3PresignedURLRequest) (entity.S3PresignedURLResponse, error) {
...
func (b *IncrementalBackupDaemon) Find(ctx context.Context, request entity.FindRequest) (map[string]interface{}, error) {
	return b.GetBackupStats(ctx, "", request.TimeStamp, "", INCREMENTAL)
}

func (b *IncrementalBackupDaemon) GetHealth(ctx context.Context, _ string) (entity.HealthResponse, error) {
...
func (b *IncrementalBackupDaemon) UpdateEvictionPolicy(_ context.Context, request entity.EvictionPolicyRequest) error {
...
func (b *IncrementalBackupDaemon) TerminateBackup(_ context.Context, request entity.TerminateRequest) error {
...
func (b *IncrementalBackupDaemon) GetQueueSize() int {
	return b.scheduler.QueueSize()
}

func (b *IncrementalBackupDaemon) DownloadBackup(_ context.Context, backupID string) (string, error) {
...
func (b *IncrementalBackupDaemon) evict(items []entity.Vault, rules string, exclude map[int64]bool) ([]entity.Vault, error) {
