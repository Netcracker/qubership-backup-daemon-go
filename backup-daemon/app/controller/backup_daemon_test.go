...
// --- BackupDaemonV2 tests ---

func newTestBackupDaemonV2(t *testing.T, ctrl *gomock.Controller) (
	*BackupDaemonV2,
	*repo.MockStorageRepository,
	*MockDBRepository,
	*utils.MockS3ClientRepository,
	*tasks.MockCommandExecutor,
	...
	bd.BackupDaemon.resolveRestoreVault = bd.resolveRestoreVault
...
func TestV2_GetBackupStats_GranularBlobPath(t *testing.T) {
	...
	bd, storageRepo, dbRepo, s3Client, executor := newTestBackupDaemonV2(t, ctrl)

	const vaultName = "20260730T061234"
	const blobPath = "backup-storage/granular"
	const vaultFolder = "backup-storage/granular/" + vaultName + "/s3-processing/" + vaultName
	...
	storageRepo.EXPECT().GetVault(vaultName, false, "", blobPath, false).Return(entity.Vault{
		Folder:     vaultFolder,
		IsGranular: true,
	})
	storageRepo.EXPECT().LoadMetrics(gomock.Any()).Return(map[string]interface{}{}, nil)
	executor.EXPECT().GetBackupDBs(vaultFolder).Return(nil, errors.New("exit status 1"))
	s3Client.EXPECT().ListCommonPrefixes(gomock.Any(), blobPath+"/"+vaultName).Return([]string{
		blobPath + "/" + vaultName + "/db1/",
		blobPath + "/" + vaultName + "/db2/",
	}, nil)
	storageRepo.EXPECT().HasCustomVars(gomock.Any()).Return(false)
...
func TestV2_CreateS3PresignedURL_BlobPath(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	bd, _, dbRepo, s3Client, _ := newTestBackupDaemonV2(t, ctrl)
	...
	s3Client.EXPECT().ListFiles(gomock.Any(), blobPath+"/"+vaultName).Return([]string{
		blobPath + "/" + vaultName + "/dump.tar.gz",
		blobPath + "/" + vaultName + "/meta.json",
	}, nil)
	s3Client.EXPECT().CreatePresignedUrl(gomock.Any(), blobPath+"/"+vaultName+"/dump.tar.gz", 3600).Return("https://s3.example.com/presigned", nil)
	...
	if len(resp.Urls) != 1 || resp.Urls[0] != "https://s3.example.com/presigned" {
		t.Fatalf("unexpected urls: %v", resp.Urls)
	}
}
